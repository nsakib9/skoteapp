    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('total_amount', function ($trips) {
                if($trips->status != 'Completed' && $trips->status != 'Cancelled'){
                    return html_entity_decode($trips->currency->symbol)."0.00";
                }
            ...
            ->addColumn('action', function ($trips) {
                $edit = '<a href="'.url(LOGIN_USER_TYPE.'/view_trips/'.$trips->id).'" class="btn btn-xs btn-primary"><i class="bx bx-bullseye"></i></a>';
