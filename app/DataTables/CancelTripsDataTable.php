...
class CancelTripsDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ->of($query)->addColumn('cancellation_fee', function ($trips) use ($default_currency) {
          
             $fee =   $this->helper->currency_convert($trips->code,$default_currency->code,$trips->cancellation_fee);

                    return $this->currency_symbol.@($fee);
    ...
    public function query(Cancel $model)
    {
        $cancel = $model->where(function($query)  {
                        ...
                        ->select(['cancel.id as id', 'cancel.created_at', 'u.first_name as driver_name', 'users.first_name as rider_name','cancel_reasons.reason as cancel_reason', 'cancel.cancel_comments as cancel_comments ','cancel.cancelled_by as cancelled_by', 'cancel.created_at as cancel_created_at','cancel.*','trips.pickup_location as pickup_location','trips.drop_location as drop_location','trips.created_at as trips_created_at','companies.name as company_name','trips.cancellation_fee as cancellation_fee','currency.code as code']);
        return $cancel;
    }
    ...
    protected function getColumns()
    {
        ...
        return array_merge($columns,[
            ...
            ['data' => 'cancellation_fee', 'name' => 'cancellation_fee', 'title' => 'Cancellation Fee'],
