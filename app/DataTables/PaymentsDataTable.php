...
class PaymentsDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('driver_or_company_commission', function ($trips) {
             ...
             ->addColumn('surcharge_amount', function ($trips) {
                return html_entity_decode(@$trips->currency->symbol).$trips->surcharge_amount;
            })
            ->addColumn('cancellation_fee', function ($trips) {
                return html_entity_decode(@$trips->currency->symbol).$trips->cancellation_fee;
            })
    ...
    public function query(Trips $model)
    {
        $trips = $model->where(function($query)  {
                        ...
                        ->select(['trips.id as id','trips.begin_trip as begin_trip', 'u.first_name as driver_name','rider.first_name as rider_name','trips.time_fare AS time_fare', 'trips.distance_fare AS distance_fare','trips.base_fare AS base_fare','trips.tips AS tips','trips.toll_fee AS toll_fee','trips.access_fee AS access_fee','trips.surcharge_amount AS surcharge_amount','trips.cancellation_fee AS cancellation_fee','trips.total_fare AS total_fare','trips.driver_payout AS driver_amount','trips.payment_status','trips.*','trips.created_at as trip_date','companies.name as company_name']);
        return $trips;
    }
    ...
    protected function getColumns()
    {
        ...
        $col_list_3 = [
            ['data' => 'surcharge_amount', 'name' => 'surcharge_amount', 'title' => 'Surcharge Amount'],
