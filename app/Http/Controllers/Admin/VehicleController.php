...
class VehicleController extends Controller
{
        ...
        if($request->submit) {
            ...
            if($vehicle_doc->count()){
                ...
                foreach($vehicle_doc as $key => $value) {
                    ...
                    $file_name = $document_name."_".time();
                    ...
                    $vehicle_doc->document = $upload_result['path'];
    ...
    public function update(Request $request)
    {
        if($request->isMethod("GET")) {
            ...
            if($data['result']) {
                ...
                $data['car_type']           = CarType::where('status','Active')->get();
                $data['company']            = Company::where('status','Active')->pluck('name','id');
                $data['path']               = storage_url('users/'.$request->id);
            ...
            if($vehicle_doc){
                foreach ($vehicle_doc as $key => $value) {
                    if($request->hasFile('file_'.$value->id)){
                        ...
                        $user_doc->document = $upload_result['path'];
                        $document_status = $value->doc_name."_status";
    ...
    public function get_driver(Request $request,$company_id) {
        ...
        $drivers = User::select('id','first_name','last_name','country_code')->where('user_type','Driver')->where('company_id',$company_id)->get();
