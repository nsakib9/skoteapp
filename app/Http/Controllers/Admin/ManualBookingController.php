...
class ManualBookingController extends Controller {
    ...
    public function store(Request $request) {
        ...
        $total_distance = $get_fare_estimation['distance'];
        ...
        if($toll_details['message'] != ''){
            flashMessage('danger', $toll_details['message']);
            return redirect(LOGIN_USER_TYPE.'/later_booking');
        }

        $surcharge_amount = $this->request_helper->surcharge_amount($polyline)[0];

        $toll_fee = ($toll_details['toll_cost'] != 0.0) ? $this->helper->currency_convert($toll_details['currency'],DEFAULT_CURRENCY,$toll_details['toll_cost']) : 0.00;
        ...
        $fare_estimation = numberFormat($base_fare + round($fare_details->per_min * $travel_minutes) + $toll_fee + $surcharge_amount);
    ...
    public function search_cars(Request $request)
    {
        ...
        if ($toll_details['message'] != '') {
            return response()->json([
                'status_message' => "toll_error",
                'trans_message' => $toll_details['message'],
                'status_code' => '0',
            ]);
        }

        $surcharge_amount_original = $this->request_helper->surcharge_amount($polyline);
        $surcharge_amount = $surcharge_amount_original[0];

        $location_name = '';

        if (!empty($surcharge_amount_original[1])) {
            $location_details = Location::whereIn('id',$surcharge_amount_original[1])->get()->pluck('name')->toArray();
            $location_name = implode(',',$location_details);
        }

        $toll_cost = ($toll_details['toll_cost'] != 0.0) ? $this->helper->currency_convert($toll_details['currency'],DEFAULT_CURRENCY,$toll_details['toll_cost']) : 0.00;

        if($request->pickup_location == $request->drop_location){
            return response()->json([
        ...
        if ($get_fare_estimation['status'] == "success") {
            ...
            $km = str_replace(",","",$get_fare_estimation['distance']);
            $location = [];
            $i = 0;
            foreach ($nearest_car as $key => $list_car) {
                ...
                $car_array[$list_car[0]->car_id] = [ 
                    ...
                    'currency' =>$currency,
                    'no_of_tolls'=>$toll_details['no_of_tolls'],
                    'toll_cost'=>$toll_cost,
                    'surcharge_amount'=>(string) $surcharge_amount,
    ...
    public function driver_list(Request $request)
    {
        ...
        $location_cars = ManageFare::where('location_id',$match_location->id)->get()->toArray();
        $location_id = $match_location->id;
        $list_car = DriverLocation::select('*')
            ->includingDistanceFromPoint($request->pickup_latitude, $request->pickup_longitude)
            ->havingRaw('
                CASE
