...
class UserController extends Controller
{
    ...
    public function updatePayoutPreference(Request $request)
    {
        ...
        if ($request->hasFile('document')) {
            ...
            $target_dir = 'driver/'.$user_id.'/uploads';
            $file_name = "payout_document_".time();
        ...
        if($request->hasFile('additional_document')) {
            ...
            $target_dir = 'driver/'.$user_id.'/uploads';
            $file_name = "payout_additional_document_".time();
            ...
            $add_document_path = $upload_result['path'];
        }
        ...
        if ($payout_method == 'Stripe') {
            ...
            if ($request->hasFile('document')) {
				$document_result = $payout_service->uploadDocument(
				...
				$payout_service->attachDocumentToRecipient(
					$recipient->id,
            ...
            if ($request->hasFile('additional_document')) {
				$add_document_result = $payout_service->uploadDocument(
					$request->file('additional_document')->getPathname(),
					$recipient->id
				);
        ...
        if (isset($document_path)) {
            ...
            $payout_preference->document_image  = $document_path;
        }
        if (isset($add_document_path)) {
            $payout_preference->additional_document_id     = $add_stripe_document ?? '';
            $payout_preference->additional_document_image  = $add_document_path;
        }
