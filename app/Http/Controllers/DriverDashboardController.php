...
class DriverDashboardController extends Controller
{
    ...
    */
    ...
    public function edit_vehicle(Request $request)
    {
        ...
        //dd(json_decode($documents),$documents);
    ...
    public function update_vehicle(Request $request) {
        ...
        if($request->vehicle_id) {
            $rules['vehicle_number'] = 'required|unique:vehicle,vehicle_number,'.$request->vehicle_id;
        } else {
        ...
        if($request->vehicle_id ==''){            
            ...
            $vehicle_documents = UserDocuments('Vehicle',$user,$request->vehicle_id);
        ...
        if($request->vehicle_id == ''){
            ...
            if(implode(',', $request->vehicle_type)!=$vehicle->vehicle_id) {
                $type_update = 1;
            }
        ...
        if($vehicle_doc->count()){
            ...
            foreach ($vehicle_doc as $key => $value){
                if($request->hasFile($value->doc_name)){
                    ...
                    $upload_result = $image_uploader->upload($document, $target_dir, $file_name);
                    ...
                    $vehicle_doc_update->document   = $upload_result['path'];
    ...
    */
    public function profile_upload(Request $request)
    {
        ...
        $image_uploader = resolve('App\Contracts\ImageHandlerInterface');
        $target_dir = 'users/'.$user->id;
        $file_name = "profile_pic_".time();
        ...
        $user_profile_image->src = $upload_result['path'];
        $user_profile_image->save();

        return [
            'success' => 'true',
            'profile_url' => storage_url($upload_result['path']),
    ...
    public function documents(Request $request) {
        ...
        $data['image'] = $user->profile_picture->src;
