...
class DriverController extends Controller
{
	...
	public function updateLocation(Request $request)
	{
		...
		$previous_location = DriverLocation::where('user_id', $user_details->id)->first();

		if ($request->trip_id) {
			$trip = Trips::where('id', $request->trip_id)->first();

			if ($user->device_type == 3) {
				$distance_travelled = getDistanceBetweenPoints(
					$previous_location->latitude, $previous_location->longitude,
					$request->latitude, $request->longitude
				);
			} else {
				$distance_travelled = $request->total_km;
			}

			$logger = new TripRouteLogger($trip);
			$logger->append($request->latitude, $request->longitude);

			$trip->total_km += $distance_travelled;
			$trip->save();

			DriverLocation::updateOrCreate(['user_id' => $user_details->id], [
		...
		if ($previous_location && $previous_location->status == 'Trip') {
			return response()->json([
				'status_code' => '0',
		...
		if($previous_location && $previous_location->status == "Pool Trip") {
			$data['pool_trip_id'] = $previous_location->pool_trip_id;
		...
		if(isset($user_details->vehicle->vehicle_id)) {
			$vehicle_list = explode(',', $user_details->vehicle->vehicle_id);
		} else {
			$vehicle_list = [];
		}
	...
	public function updateVehicle(Request $request) {
		...
		$type_update = $other_update = 0;

		if(!$request->id) {
			$vehicles =  new Vehicle;

			$type_update = $other_update = 1;
		} else {
			...
			if($request->vehicle_type!=$vehicles->vehicle_id) {
	        	$type_update = 1;
	        }
	...
	public function update_document(Request $request)
	{
		...
		if($request->hasFile('document_image')) {
			...
			$target_dir 	= 'users/'.$user_details->id;
			$image 			= $request->file('document_image');
			$file_name 		= $documents->doc_name."_".time();
		    ...
		    if(!$upload_result['status']) {
		   	...
		   	$driver_document->document = $upload_result['path'];
		}
		...
		return response()->json([
			...
			'document_url' 			=> $driver_document->document ? storage_url($driver_document->document) : '',
			'document_status'		=> 0,
		]);
	}
}
