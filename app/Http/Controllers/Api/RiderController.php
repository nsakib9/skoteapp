...
class RiderController extends Controller
{
    ...
    public function searchCars(Request $request)
    {
        ...
        $cancel_trips = Trips::where('user_id',$user_details->id)->where('is_paid','Not Paid')->where('cancellation_fee','>','0.00')->first();
        if(!empty($cancel_trips)){
            return response()->json([
                'status_code' => '0',
                'status_message' => trans('messages.payment_pending'),
                'trip_id' =>$cancel_trips->id,
            ]);
        }
        ...
        $match_location = Location::select('id', 'status')
            ->active()
            ->containingPoint($request->pickup_latitude, $request->pickup_longitude)
            ->first();
        ...
        $toll_cost = ($toll_details['toll_cost'] != 0.0) ? $this->helper->currency_convert($toll_details['currency'],$user_details->currency_code,$toll_details['toll_cost']) : 0.00;

        $surcharge_amount_orginal = $this->request_helper->surcharge_amount($polyline);
        $surcharge_amount =  $this->helper->currency_convert(DEFAULT_CURRENCY,$user_details->currency_code,$surcharge_amount_orginal[0]);
        ...
        $nearest_car = DriverLocation::select('*')
            ->includingDistanceFromPoint($request->pickup_latitude, $request->pickup_longitude)
            ...
            ->havingRaw('
                CASE
                    WHEN status = "Online" THEN distance <= ?
                    ELSE distance <= ?
                END
            ', [site_settings('driver_km'), site_settings('pickup_km')])
            ...
            $nearest_car = $nearest_car->filter(function($near_car) use($lat_lng_array) {
                ...
                $trip_destinations = PoolTrip::with(['trips' => function($query) use($lat_lng_array) {
                    $query
                        ...
                        ->selectRaw('
                            CASE
                                WHEN status IN ("Scheduled", "Begin trip", "End Trip") THEN (
                                    3959 * ACOS(
                                ...
                                ELSE 999999999
                        END AS distance', [
                            $lat_lng_array['drop_latitude'],
                            $lat_lng_array['drop_longitude'],
                            $lat_lng_array['drop_latitude'],
                        ])
        ...
        $get_fare_estimation =   $this->request_helper->GetDrivingDistance_mile($request->pickup_location, $request->drop_location);
        ...
        $km = $get_fare_estimation['distance'];
        ...
        if (isset($nearest_car) && !$nearest_car->isEmpty()) {
            ...
            $data = ManageFare::with([
                'peak_fare' => function($query) use($day,$current_time) {
                    ...
                    ->orWhere(function($q) use($day,$current_time) {
                        $q
                            ->where('day', null)
                            ->whereRaw('(SELECT CASE
                                WHEN start_time < end_time THEN (
                                    start_time <= ? AND end_time >= ?
                                )
                                ELSE (
                                    (? BETWEEN start_time AND "23:59:00") OR
                                    (? BETWEEN "00:00:00" AND end_time)
                                )
                            END)', array_fill(0, 4, $current_time));
                    });
                }
            ])
            ...
            foreach ($nearest_car as $key => $list_car) {
                ...
                if (count($location) > 0) {
                    ...
                    $km = (str_replace(",","",$km) != '') ? str_replace(",","",$km) : 1;
                ...
                $fare_estimation  = $fare_estimation + $toll_cost + $surcharge_amount;
                ...
                $car_array[$list_car[$i]->car_id] = array(
                    ...
                    'minutes'       => $minutes,
                    'time_fare'     =>  number_format((round($list_car[$i]->car_type->manage_fare->per_min * $minutes)), 2, '.', ''),
                    'distance_fare' => number_format((round($list_car[$i]->car_type->manage_fare->per_km * $km)), 2, '.', ''),
                    ...
                    'no_of_tolls'=>(string) $toll_details['no_of_tolls'],
                    'toll_cost'=>(string) $toll_cost,
                    'surcharge'=>(string) $surcharge_amount,
                    'location_name'=>$location_name,
                    'black_car_fund'=>fees('black_car_fund'),
                    'sales_tax'=>fees('sales_tax'),
                    'get_additional_tax' => get_additional_tax(),
                );
            }
        }
        ...
        foreach($cars as $key => $value) {
            ...
            $car_array[$value->id] = [
                ...
                'minutes'       => $minutes,
                'time_fare'     =>  number_format((round($value->manage_fare->per_min * $minutes)), 2, '.', ''),
                'distance_fare' => number_format((round($value->manage_fare->per_km * $km)), 2, '.', ''),
                ...
                'location' 		=> array(),
                'drivers' 		=> array(),
                'fare_estimation' => number_format($fare_estimation,2),
                'additional_rider_percentage' => $additional_rider,
                'min_time'		=> 'No cabs',
                "apply_peak"	=> "No",
                "peak_price" 	=> 0,
    ...
    public function get_nearest_vehicles(Request $request)
    {
        ...
        // Find nearest cars in location
        $nearest_car = DriverLocation::select('*')
            ->includingDistanceFromPoint($request->latitude, $request->longitude)
            ->with('car_type', 'users')
            ->where(function($q) {
            ...
            ->whereHas('car_type', function($q) {
                $q
                    ->where('status', 'Active')
                    ->whereRaw('CASE
                        WHEN driver_location.status = "Pool Trip" THEN car_type.is_pool = "Yes"
                        ELSE 1
                    END');
            })
            ->havingRaw('
