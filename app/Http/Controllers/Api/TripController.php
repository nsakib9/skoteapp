...
use App\Models\Fees;
use App\Services\TripRouteLogger;
use DateTime;
use DB;
use File;
use JWTAuth;
...
class TripController extends Controller
{
	...
	protected function getTripDetails($trip_id,$user,$is_trip_id=false,$response=true,$riders_only=false) {
		...
		if($trip->pool_id>0 && !$is_trip_id) {
			$pool_trip = PoolTrip::with(['trips' => function($query) use($user) {
				if($user->user_type == 'Rider') {
                    ...
                    $driver_location = $query->get()[0]->driver_location;
                    $query
                        ->with('driver_location')
                        ->select('*')
                        ->selectRaw('(CASE
                            WHEN status IN ("Scheduled", "Begin trip") THEN (
                                3959 * ACOS(
                                    COS(RADIANS(?)) * COS(RADIANS(pickup_latitude)) * COS(
                            ...
                            ELSE (
                                3959 * ACOS(
                                    COS(RADIANS(?)) * COS(RADIANS(drop_latitude)) * COS(
                                        RADIANS(drop_longitude) - RADIANS(?)
                                    ) + SIN(RADIANS(?)) * SIN(RADIANS(drop_latitude))
                                )
                            )
                        END) as distance', [
		...
		if(count($trips)) {
			foreach($trips as $tkey=>$trip) {
                ...
                $data[$tkey]['is_trip_cancel_paid'] = ($trip->is_paid == 'Not Paid' && $trip->cancellation_fee != 0.00) ? true : false;
                $data[$tkey]['cancellation_fee'] = $trip->cancellation_fee;
                ...
                $data[$tkey]['arrive_now']      = $trip->arrive_time;
                ...
                $data[$tkey]['total_fare'] = ($trip->status != 'Completed' && $trip->status != 'Cancelled') ? "0.00" : $total_fare;

				$driver_earnings = $symbol.number_format($trip->company_driver_earnings,2);
                $data[$tkey]['driver_earnings']= ($trip->status != 'Completed' && $trip->status != 'Cancelled') ? $symbol."0.00" : $driver_earnings;
                $data[$tkey]['driver_payout'] = ($trip->status != 'Completed' && $trip->status != 'Cancelled') ? "0.00" : $trip->driver_payout;
	...
	public function end_trip(Request $request)
	{
		...
		$messages = array(
            'trip_path.required' => 'Trip path is required',
		);
        ...
        if($request->trip_path == ''){
            $response = $this->request_helper->GetPolyline($trip->pickup_latitude, $request->end_latitude, $trip->pickup_longitude, $request->end_longitude);
        } else {

            $trip_path = explode('|',$request->trip_path);
            $trip_path = array_filter($trip_path);

            foreach($trip_path as $path){
                $data = explode(',',$path);
                $val[] = ['lat'=>$data[0],'lng'=>$data[1]];
            }

            $response =  \GeometryLibrary\PolyUtil::encode($val);
        }
        ...
        $toll_details = toll_guru($data1);

        if($toll_details['message'] != '' && $toll_details['message'] != 'Internal server error'){
            return response()->json([
                'status_message' => $toll_details['message'],
                'status_code' => '0',
            ]);
        }

        $toll_fee = (@$toll_details['toll_cost'] != 0.0) ? $this->helper->currency_convert($toll_details['currency'],$trip->currency_code,$toll_details['toll_cost']) : 0.00;

        $surcharge_amount_orginal = $this->request_helper->surcharge_amount($response);

        $surcharge_amount =  $this->helper->currency_convert(DEFAULT_CURRENCY,$trip->getOriginal('currency_code'),$surcharge_amount_orginal[0]);

        $location_name = '';
        if(!empty($surcharge_amount_orginal[1])){
            $location = Location::whereIn('id',$surcharge_amount_orginal[1])->get()->pluck('name')->toArray();
            $location_name = implode(',',$location);
        }
		...
		if($request->trip_id) {
            $total_km = number_format(($request->total_km/1.609),5,'.','0');
			$data = [
		...
		if ($request->hasFile('image')) {
			...
			$target_dir = 'map/'.$trip->id;
			$image 		= $request->file('image');
            $file_name 	= substr(md5(uniqid(rand(), true)), 0, 8);
            $upload_result = $image_uploader->upload($image, $target_dir, $file_name);
            ...
            $image_url = storage_url($upload_result['path']);
		}
		...
		$trip_data = [
			...
			'end_trip' 		=> $end_time,
			'total_km' 		=> $total_km,
			'map_image' 	=> isset($upload_result) ? $upload_result['path'] : '',
			'toll_fee'		=> $toll_fee,
            'surcharge_amount' => $surcharge_amount,
            'surcharge_location'=> $location_name,
		];
        ...
        $logger = new TripRouteLogger($trip);
        $logger->append($request->end_latitude, $request->end_longitude);
        $logger->complete();
	...
	public function map_upload(Request $request)
	{
        ...
        Trips::where('id', $request->trip_id)->update(['map_image' => $upload_result['path']]);
	...
	public function cancel_trip(Request $request)
	{
		...
		if ($user_type == 'rider') {
            ...
            $from = \Carbon\Carbon::createFromFormat('Y-m-d H:i:s',  $cancelled_id->created_at);
            $to = \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', date('Y-m-d H:i:s'));
            $diff_in_minutes = $to->diffInMinutes($from);

            if($diff_in_minutes >= site_settings('update_cancellation_interval')){
                $cancellation_fee = fees('cancellation_fee');
                $cancel_fee = $this->helper->currency_convert(DEFAULT_CURRENCY,$cancelled_id->getOriginal('currency_code'),$cancellation_fee);
                $is_cancel = 1;
            }
		}
		else {
        ...
        Trips::where('id', $request->trip_id)->update(['status' => 'Cancelled','cancellation_fee'=>$cancel_fee, 'payment_status' => 'Trip Cancelled','total_fare'=>$cancel_fee]);
	...
	public function get_trip_details(Request $request)
	{
        ...
        if($trip_id == '') {
            ...
            $pending_trips = Trips::where('user_id',$user_details->id)->where('status','Cancelled')->where('cancellation_fee','!=','0.00')->where('is_paid','Not Paid')->first();
            if(!empty($pending_trips)){
                $trip_id = $pending_trips->id;
            }
	...
	public function acceptTrip(Request $request)
	{
		...
		$firbase->deleteReference("trip_request/".$user_details->id);
