...
class HomeController extends Controller
{
    ...
    public function check_version(Request $request)
    {
        ...
        return array(
            ...
            'force_update'		=> !static::satisfiesMobileAppVersionConstraints($request->version),
            'enable_referral'	=> ($referral_settings->value == "1"),
    ...
    /**
     * Checks that the given version string satifies the minimum app version
     * defined in this app's configuration (see `config/misc.php`).
     *
     * This is currently used to check the version of mobile apps connecting to
     * the API so that we can force users to update them.
     *
     * @param string $version
     * @return boolean
     */
    protected static function satisfiesMobileAppVersionConstraints($version) {
        $constraints = config('misc.mobile_app_version_constraints');
        return $constraints && Semver::satisfies($version, $constraints);
    }
}
