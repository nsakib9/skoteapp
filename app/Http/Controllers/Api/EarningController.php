...
class EarningController extends Controller
{
	...
	public function afterPayment(Request $request)
	{
		...
		if($trip->status != 'Payment' && $trip->cancellation_fee == 0.00) {
			return response()->json([
				'status_code' => '0',
		...
		if($trip->total_fare > 0) {
            ...
            $payment_data['amount'] = ($trip->cancellation_fee > 0.00) ? $trip->cancellation_fee : $trip->total_fare;
			...
			try {
				...
				if(!$pay_result->status) {
                    if($trip->cancellation_fee > 0.00){
                        $trip->is_paid = 'Not Paid';
                    } else {
		...
		return response()->json([
            ...
            'cancellation_fee'  => $trip->cancellation_fee ?? '0.00',
