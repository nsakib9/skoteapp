...
class PayoutDetailController extends Controller
{
	...
	*/
	public function weeklyStatement(Request $request)
	{
        ...
        if($trips->sum('toll_fee') > 0){
            $toll_fee = array(
                "key" => __('messages.toll_fee'),
                "value" => $symbol.number_format($trips->sum('toll_fee'),2),
            );
            $content[] = formatStatementItem($toll_fee);
        }

        if($trips->sum('surcharge_amount') > 0){
            $surcharge_amount = array(
                "key" => __('messages.surcharge_amount'),
                "value" => $symbol.number_format($trips->sum('surcharge_amount'),2),
            );
            $content[] = formatStatementItem($surcharge_amount);
        }
	...
	*/
    ...
    public function updatePayoutPreference(Request $request)
    {
        ...
        if ($payout_method == 'Stripe') {
            ...
            if ($request->hasFile('document')) {
                $document_path = $request->file('document')->getPathname();
