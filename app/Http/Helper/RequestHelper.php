...
class RequestHelper
{
    ...
    public function find_driver($array)
    {
        ...
        */
        ...
        $nearest_car = DriverLocation::select('*')
            ->includingDistanceFromPoint($array['pickup_latitude'], $array['pickup_longitude'])
            ->havingRaw('
                CASE
            ...
            $trip_destinations = PoolTrip::with(['trips' => function($query) use($data_array) {
                $query
                    ...
                    ->selectRaw('CASE
                        ...
                        ELSE 999999999
                    END AS distance', [
                        $data_array['drop_latitude'],
                        $data_array['drop_longitude'],
                        $data_array['drop_latitude'],
                    ])
    ...
    public function check_inside_polygon($value){
        $location = Location::where('status','Active')->get();
        $local = '';
        foreach($location as $locas){
            $response =  \GeometryLibrary\PolyUtil::containsLocation(
                $value, // point array [lat, lng]
                $locas->coordinates[0],
            );
            if($response == true){
                $local = $locas->id;
    ...
    public function surcharge_amount($trip_path){
        $response =  \GeometryLibrary\PolyUtil::decode($trip_path);
        $check_inside_polygon = [];
        foreach($response as $value){
            $check_inside_polygon[] = $this->check_inside_polygon($value);
        }

        $location_id = array_unique(array_filter($check_inside_polygon));
        $subcharge = 0.00;
        if(!empty($location_id)){
            $subcharge = Location::whereIn('id',$location_id)->sum('surcharge_amount');
        }

        return array($subcharge,$location_id);
    }
    ...
    public function GetDrivingDistance_mile($lat1, $lat2)
    {
        $google_service = resolve("google_service");
        return $google_service->GetDrivingDistance_mile($lat1, $lat2);
    }
