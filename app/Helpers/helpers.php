...
if (!function_exists('toll_guru')) {
    function toll_guru($data) {
        $curl = curl_init();
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_HTTPHEADER => array(
                ...
                "x-api-key:".api_credentials('toll_guru','TollGuru'),
            ),
        ));
        ...
        if ($err) {
            ...
            $no_of_tolls = (!empty($details->route->tolls)) ? count($details->route->tolls) : 0;
            $currency = "";
            if(!empty($details->route->tolls)){
                foreach($details->route->tolls as $tolls){
                    $cost[] = $tolls->tagPriCost;
                    $currency = $tolls->currency;
                }
                $toll_cost = array_sum($cost);
            } else{
                $toll_cost = 0.00;
            }
            return [
                'currency' => $currency,
                'no_of_tolls' => @$no_of_tolls,
                'toll_cost' => @$toll_cost,
...
if (!function_exists('get_additional_tax')) {
    function get_additional_tax()
    {
        return AdditionalTax::where('status','Active')
            ->where('value','!=','0')
            ->get()
            ->map(function($val) {
                return [
...
if (!function_exists('getAppleClientSecret')) {
	...
	{
		$key_file = resource_path() . '/credentials/apple.key';
...
if (!function_exists('getDistanceBetweenPoints')) {
    function getDistanceBetweenPoints($latFrom, $lonFrom, $latTo, $lonTo, $earthRadius = 3959) {
...
if (!function_exists('storage_url')) {
    /**
     * We use local filesystem storage in development, but S3 in production.
     * This method generates the appropriate URL for an asset path in the
     * current environment.
     */
