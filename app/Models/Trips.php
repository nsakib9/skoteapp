...
class Trips extends Model
{
    ...
    public $convert_fields = ['time_fare', 'distance_fare', 'base_fare', 'total_fare', 'access_fee','black_car_fund','sales_tax','driver_payout', 'owe_amount', 'remaining_owe_amount', 'applied_owe_amount', 'wallet_amount','promo_amount','payable_driver_payout','cash_collectable','commission','company_admin_commission','total_trip_fare','total_invoice','total_payout_frontend','cash_collect_frontend','driver_front_payout','rider_paid_amount','subtotal_fare','peak_amount','schedule_fare','driver_peak_amount','company_commission','driver_service_fee','driver_or_company_commission','driver_or_company_earning','tips','admin_total_amount', 'waiting_charge','toll_fee','driver_earnings','additional_rider_amount','additional_tax','surcharge_amount','cancellation_fee'];
    /**
    ...
    public function getStatusAttribute()
    {
        ...
        if($status == 'Cancelled' && $this->attributes['cancellation_fee'] != '0.00' && $this->attributes['is_paid'] == 'Not Paid' && (@Auth::user()->user_type=="Rider" || LOGIN_USER_TYPE == 'company' || LOGIN_USER_TYPE == 'admin')){
            return trans('messages.home.cancel_payment');
        }
    ...
    {
        return $this->attributes['access_fee'] +$this->attributes['additional_tax'] + $this->attributes['black_car_fund'] + $this->attributes['sales_tax'] + ( $this->attributes['peak_amount'] - $this->attributes['driver_peak_amount'] ) + $this->attributes['schedule_fare'] + $this->attributes['driver_or_company_commission'];
    }
    ...
    {
        return ( $this->attributes['subtotal_fare'] + $this->attributes['driver_peak_amount'] + $this->attributes['tips'] + $this->attributes['waiting_charge'] + $this->attributes['toll_fee'] + $this->attributes['surcharge_amount'] + $this->attributes['additional_rider_amount']) - $this->attributes['driver_or_company_commission'];
    }

    public function getAdminTotalAmountAttribute()
    {
        return ( $this->attributes['subtotal_fare'] + $this->attributes['peak_amount'] + $this->attributes['access_fee'] + $this->attributes['additional_tax'] + $this->attributes['black_car_fund'] + $this->attributes['sales_tax'] + $this->attributes['schedule_fare'] + $this->attributes['tips'] + $this->attributes['toll_fee'] + $this->attributes['surcharge_amount'] + $this->attributes['cancellation_fee'] + $this->attributes['waiting_charge']);
    }
    ...
    public function getPayableDriverPayoutAttribute()
    {
        ...
        if(($this->attributes['payment_mode']=="Cash" || $this->attributes['payment_mode']=="Cash & Wallet") && ($this->attributes['wallet_amount']!=0 || $this->attributes['promo_amount']!=0)) {
            ...
            return number_format(($this->attributes['driver_payout'] + $this->attributes['access_fee'] + $this->attributes['additional_tax']+$this->attributes['black_car_fund'] + $this->attributes['sales_tax'] -$cash_collectable),2, '.', '');
        }

        return number_format((($this->total_fare()-$this->attributes['access_fee']-$this->attributes['additional_tax']-$this->attributes['black_car_fund'] - $this->attributes['sales_tax'])-$this->attributes['applied_owe_amount']),2, '.', '');
    }
    ...
    {
        return $total_fare = $this->attributes['base_fare'] + $this->attributes['time_fare'] + $this->attributes['distance_fare'] + $this->attributes['schedule_fare']  + $this->attributes['access_fee'] + $this->attributes['additional_tax'] + $this->attributes['sales_tax'] + $this->attributes['black_car_fund'] + $this->attributes['peak_amount'] + $this->attributes['tips'] + $this->attributes['waiting_charge'] + $this->attributes['toll_fee'] + $this->attributes['surcharge_amount'];
    }
    public function getDriverFrontPayoutAttribute()
    {
        return number_format((($this->attributes['wallet_amount']+$this->attributes['promo_amount'])-($this->attributes['access_fee'] + $this->attributes['additional_tax'] + $this->attributes['sales_tax'] + $this->attributes['black_car_fund'] +$this->attributes['applied_owe_amount'])),2, '.', '');
    }
    ...
    {
        return $this->getUserProfileImage('rider');
    }
    // get DriverThumb image
    public function getDriverThumbImageAttribute()
    {
        return $this->getUserProfileImage('driver');
    }
    // get DriverThumb image
    ...
    {
        return number_format(($this->attributes['total_fare'] -$this->attributes['access_fee']- $this->attributes['sales_tax'] - $this->attributes['black_car_fund'] - $this->attributes['additional_tax'] ),2, '.', '');
    }
    ...
    {
        return number_format(($this->attributes['cancellation_fee']),2, '.', ''); 
    }
    ...
    {
        return number_format(($this->attributes['surcharge_amount']),2, '.', ''); 
    }
    ...
    {
        return number_format(($this->attributes['sales_tax']),2, '.', ''); 
    }
     ...
     *
     */
    public function getBlackCarFundAttribute()
    {
        return number_format(($this->attributes['black_car_fund']),2, '.', ''); 
    }
     ...
     *
     */
    public function getAdditionalTaxAttribute()
    {
        return number_format(($this->attributes['additional_tax']),2, '.', '');
    }
    ...
    protected function getUserProfileImage($type)
    {
        switch ($type) {
            case 'driver':
                $param = 'driver_id';
            ...
            case 'rider':
                ...
                break;
            default:
                throw \InvalidArgumentException('$type must be "driver" or "rider"');
        }

        $profile_picture = ProfilePicture::where('user_id' ,$this->attributes[$param])->first();
        return isset($profile_picture) ? storage_url($profile_picture->src) : url('images/user.jpeg');
    }
}
