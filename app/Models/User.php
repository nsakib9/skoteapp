...
class User extends Authenticatable implements JWTSubject
{
    ...
    public function getIsReferralCompletedAttribute()
    {
        ...
        $referral_user = ReferralUser::where('user_id',$this->referral_user_id)
            ->where('referral_id',$this->attributes['id'])
            ->whereRaw('? between start_date and end_date', [$c_date])
            ->wherePaymentStatus('Pending')
            ->first();
    ...
    {
        $c_date = date('Y-m-d');
        $referral_user = ReferralUser::where('user_id',$referral_user_id)
            ->where('referral_id',$this->attributes['id'])
            ->whereRaw('? between start_date and end_date', [$c_date])
            ->wherePaymentStatus('Pending')
            ->first();
