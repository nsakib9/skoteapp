...
class GoogleAPIService
{
	...
	{ 
		$lat1 = str_replace(" ","+",str_replace("null","",$lat1));
		$lat2 = str_replace(" ","+",str_replace("null","",$lat2));
		

		$url = "https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=".$lat1."&destinations=".$lat2."&key=".MAP_SERVER_KEY;
		//$this->base_url."/distancematrix/json?origins=" . $lat1 ."&destinations=" . $lat2 ."&units=imperial&language=pl-PL&key=" . MAP_SERVER_KEY;
		...
		elseif ($response_a->status == "OK" ) {
			...
			$dist_find = @$response_a->rows[0]->elements[0]->distance->text;
			$time_find = @$response_a->rows[0]->elements[0]->duration->value;

			$dist = @$dist_find != '' ? str_replace("mi","",$dist_find) : '';
			$time = @$time_find != '' ? $time_find : '';
			$return_data = array('status' => 'success', 'distance' => trim($dist), 'time' => (int) $time);
	...
	public function GetPolyline($lat1, $lat2, $long1, $long2)
	{
		...
		$url = $this->base_url."/directions/json?origin=" . $lat1 . "," . $long1 . "&destination=" . $lat2 . "," . $long2 . "&units=metric&sensor=true&&language=pl-PL&key=" . MAP_SERVER_KEY;
